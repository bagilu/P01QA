(function () {
  ...
  // V26.2: 題目欄位可安全使用少量 HTML。
  // 目前允許：文字、<br>、<img>、<b>/<strong>、<i>/<em>。
  // 題目圖片限定使用 P01 repository 內 images/questions/ 的相對路徑。
  function sanitizeQuestionHtml(rawHtml) {
    const template = document.createElement('template');
    template.innerHTML = String(rawHtml ?? '');

    const allowedTags = new Set(['BR', 'IMG', 'B', 'STRONG', 'I', 'EM']);
    ...
    function cleanNode(node) {
      ...
      if (tag === 'IMG') {
        ...
        // Only permit neutral, repository-local quiz assets.
        if (!/^images\/questions\/[A-Za-z0-9._-]+\.(?:jpg|jpeg|png|webp)$/i.test(src)) {
          return document.createTextNode('[圖片路徑無效]');
        }
        const img = document.createElement('img');
        img.src = src;
        img.alt = (node.getAttribute('alt') || '題目圖片').slice(0, 200);
        img.loading = 'eager';
        img.decoding = 'async';
        return img;
      }
  ...
  function renderQuestionObject(state, question) {
    ...
    $('questionText').innerHTML = sanitizeQuestionHtml(state.question.Q);
