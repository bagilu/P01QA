...
Deno.serve(async (req) => {
  ...
  try {
    const { game_id, user_id, qid, question_no } = await req.json();
    const gameId = Number(game_id);
    const userId = String(user_id || '').trim();
    const qidNumber = Number(qid);
    const qNo = Number(question_no);

    if (!gameId || !userId || !qidNumber || !qNo) return errorResponse('缺少切換題目所需資料。');

    const supabase = getAdminClient();
    const { data: session, error: sessionError } = await supabase
      .from('TblP01GameSession')
      .select('GameID, HostUserID, Status')
      .eq('GameID', gameId)
      .single();
    if (sessionError) throw sessionError;
    if (!session) return errorResponse('找不到競賽資料。', 404);
    if (session.HostUserID !== userId) return errorResponse('只有主持者可以切換題目。', 403);
    if (session.Status === 'ended') return errorResponse('此競賽已結束。');
    ...
    console.error(err);
    return errorResponse(err?.message || '切換題目失敗。', 500);
  }
});
